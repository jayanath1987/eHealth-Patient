...
if ((isset($previous_injection_list))&&(!empty($previous_injection_list))){			
	...
	echo '<div class="panel-heading"  ><b>Previous vaccinations and injections</b></div>';
		...
		for ($i=0;$i<count($previous_injection_list); ++$i){
			...
			if ($previous_injection_list[$i]["status"] =="Pending"){
				echo ' onclick = self.document.location="'.site_url("form/edit/patient_injection/".$previous_injection_list[$i]["patient_injection_id"]).'" ';
			}
			echo '>';
			echo '<td width="10px">';
